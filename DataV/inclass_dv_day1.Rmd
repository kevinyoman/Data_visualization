...
- datetimes: trending_date, publish_time
- factor: category
...
# vids$publish_time <- as.Date(vids$publish_time, format='%Y-%m-%d')
str(vids)
```
...
vids$time_to_trend <- as.numeric(vids$time_to_trend)
vids
```
...
vids$publish_when <- factor(vids$publish_when)
str(vids)
```
...
Mari kita kurangi redudansi data yang terdapat di dataframe **vids** dan simpan dalam objeck baru bernama **vids.unik**
...
```{r}
# Please type your code down below
vids.agt <- vids.unik[vids.unik$category %in% c("Autos and Vehicles","Gaming","Travel and Events"),]
vids.agt
...
- 3 kategori diatas memiliki like_ratio terkecil
