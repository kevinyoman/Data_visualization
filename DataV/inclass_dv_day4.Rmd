...
```{r}
# versi plot()
plot(x = vids.agt$category_id,
     y = vids.agt$like_ratio)

# versi boxplot()
boxplot(formula = like_ratio~category_id, 
        data = vids.agt)
```
...
- secara general, kategori Gaming memiliki like ratio yang lebih baik dari 2 kategori lain

## Bar

Business case: Setelah mengetahui konten yang menurutnya menarik, Pertanyaan selanjutnya adalah *pada rentang jam berapa saya harus publish supaya diminati?*
...
plot(x = vids.agt$like_ratio,
     ...
     col = vids.agt$category_id,
     pch = 16)
legend(x = "topright", # hanya bisa di dalam kotak plotnya
       legend = levels(vids.agt$category_id),
       # fill = 1:3, inset=c(-0.2,0))
       fill = 1:3)
```
...
plot(x = vids.agt$like_ratio,
     ...
     col = vids.agt$category_id,
     pch = 16)
legend(x = "right", # hanya bisa di dalam kotak plotnya
       legend = levels(vids.agt$category_id),
       # fill = 1:3, inset=c(-0.2,0))
       fill = 1:3)
...
- analisa lebih lanjut konten gaming dengan dislikes tinggi
...
ggplot(data = vids.agt,
       mapping = aes(x = category_id, y = comment_ratio))

plot1 <- ggplot(data = vids.agt,
       mapping = aes(x = category_id, y = comment_ratio))
```
...
ggplot(data = vids.agt,
  ...
  geom_jitter(mapping = aes(size = like_ratio))
```
...
# Please Type Your Code Down Below
ggplot(data = vids.agt,
       mapping = aes(x = category_id, y = comment_ratio, color = category_id)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(mapping = aes(size = like_ratio))
```
...
# Please Type Your Code Down Below
ggplot(data = vids.agt,
       mapping = aes(x = category_id, y = comment_ratio, fill = category_id)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(mapping = aes(size = like_ratio))
```
...
ggplot(data = vids.agt,
       mapping = aes(x = category_id, y = comment_ratio)) +
  geom_boxplot(outlier.shape = NA, mapping = aes(color = category_id)) +
  geom_jitter(mapping = aes(size = like_ratio))
```

Inclass Q: Gimana kalo kita pengen liat nilai yang like rationya lebih dari sebuah nilai yang kita tentukan?

```{r}
ggplot(data = vids.agt,
       mapping = aes(x = category_id, y = comment_ratio)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(mapping = aes(size = like_ratio, color = like_ratio > 0.05)) +
...
ggplot(data = vids.agt,
       mapping = aes(x = category_id, y = comment_ratio)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(mapping = aes(size = like_ratio, color = ifelse(test = like_ratio > 0.05, yes = TRUE, no = FALSE))) +
  scale_color_manual(values = c('red','black')) +
  labs(color = "Like ratio > 0.05")
```
...
# Please Type Your Code Down Below
ggplot(data = vids.agt,
       mapping = aes(x = category_id, y = comment_ratio, col = "#e65e53")) +
  geom_boxplot(outlier.shape = NA, mapping = aes(color = category_id)) +
  geom_jitter(mapping = aes(size = like_ratio)) +
  labs(title = "Comment & Like Ratio",
       subtitle = "Category: Autos & Vehicles, Gaming, Travel & Events",
       y = "Comment Ratio",
       x = "Category", 
...
Inclass Q: Lawan dari %in%

```{r}
vids.agt[!(vids.agt$category_id %in% c("Gaming")),]
```

Inclass Q: Coba bahas line chart yang dipisahkan kategorinya.

> akan dibahas di hari ke-4
...
```{r}
trending_ch_sort <- trending_channel[order(trending_channel$Freq, decreasing = T),]
...
ggplot(data = head(trending_ch_sort, 10),
       mapping = aes(x = Freq, y = reorder(Var1, Freq))) +
  geom_col(mapping = aes(fill = Freq)) + 
  geom_vline(mapping = aes(xintercept = mean(Freq))) +
  scale_fill_gradient(low = "#f0bf00", high = "#58b258") +
  geom_text(mapping = aes(label = Freq), nudge_x = -3, color = "red") 
...
Buat visualisasi "Top 15 Channel in Youtube Trending", Highlight 5 channel teratas dengan label jumlah videonya lalu percantik plot sesuai keinginan Anda!

```{r}
ggplot(data = head(trending_ch_sort, 15),
       mapping = aes(x = Freq, y = reorder(Var1, Freq))) +
  geom_col(mapping = aes(fill = ifelse(test = Freq > 25, yes = "Top5", no = "Below5"))) + 
  # geom_vline(xintercept = 25) + 
  geom_text(data = head(trending_ch_sort, 5), mapping = aes(label = Freq), nudge_x = 1) +
...
ggplot(data = head(trending_ch_sort, 15),
       mapping = aes(x = Freq, y = reorder(Var1, Freq))) +
  geom_col(fill = "black") +
  geom_col(fill = "green", data = head(trending_ch_sort, 5)) +
  geom_text(data = head(trending_ch_sort, 5), mapping = aes(label = Freq), nudge_x = 1) +
  labs(y = "Channel Title",
       x = "Trending Count")
```

Inclass Q: Bahas plot untuk sistem koordinat/spatial plot.

> Nanti saya kasih referensi

* [Library Leaflet, can be used for spatial data](https://rstudio.github.io/leaflet/)
* [Plotly for spatial data](https://plotly.com/r/maps/)
...
ggplot(data = vids.agg.long,
  ...
  geom_text(mapping = aes(label = round(proportion, 3), group = variable), position = position_dodge(width = .9), hjust = 0.01) +
  scale_fill_manual(values=c('Yellow','Blue'))+
  labs(title="Likes & Dislikes \nRation of Youtube Trending Video",
       x = "Ratio",
       y = "Channel Category",
       fill = "Condition"   
      )+
...
Inclass Q: Cari beda hjust dengan nudge_x

> hjust kita gunakan jika kita mengatur position (contoh: position_dodge())
...
ggplot(data = vids.agg.long,
       mapping = aes(x = proportion, y = reorder(category, proportion))) + 
  geom_col(mapping = aes(fill = variable), position = "fill") 
```
...
* `facets` =  parameter yang menentukan kolom apa yang mau dipisah
  - contoh penulisan dalam parameter facets yang bisa digunakan:
    + facets = ~ nama_kolom
    + facets = c("nama_kolom")
    + facets = vars(nama_kolom)
...
```{r}
# Please run the code down below
vids.ces <- data.frame(table("Date" = vids.ces$trending_date, 
                             "Cat" = vids.ces$category_id))
vids.ces 
```
...
```{r}
vids.ces$Date <- ymd(vids.ces$Date)
# Please type your code down below
...
ggplot(data = vids.ces, mapping = aes(x = Date, y = Freq, color = Cat))+
  ...
  scale_x_date(date_breaks="2 week",
               date_labels="%y.%m.%d")+
  scale_y_continuous(breaks =seq(0,max(vids.ces$Freq),5)  )
```
