...
```{r}
# your code here
kiva_man <- kiva[(kiva$sector == 'Manufacturing'),]
plot(kiva_man$loan_amount,kiva_man$lender_count)
```
...
kiva <- read.csv("loan_kiva.csv")
kiva_phil <- kiva[(kiva$country =='Philippines'),]
kiva_phil$posted_time <- ymd_hms(kiva_phil$posted_time)
min(kiva_phil$posted_time)
max(kiva_phil$posted_time)
summary(kiva_phil)
```
...
kiva_phil$posted_time <- ymd_hms(kiva_phil$posted_time)
kiva_phil$posted_time <- ymd_hms(kiva_phil$posted_time)
kiva_phil$funded_time <- ymd_hms(kiva_phil$funded_time)
kiva_phil$funding_duration <- difftime(kiva_phil$funded_time, kiva_phil$posted_time)
kiva_phil$funding_duration <- as.numeric(kiva_phil$funding_duration)
kiva_phil$funding_duration <- kiva_phil$funding_duration/60 
```
...
```{r}
# your code here
kiva_phil$month <- month(kiva_phil$posted_time,label=T,abbr=T)
```
...
```{r}
# your code here
loan_agg <- aggregate(funding_duration~month+repayment_interval,kiva_phil,mean)
